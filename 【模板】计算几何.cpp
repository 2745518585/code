...
struct polygon
{
    ...
    {
        double s=0;
        if(x!=Map1.begin()&&next(x)!=Map1.end()) s-=cross(check(next(x)),check(prev(x)));
        if(x!=Map1.begin()) s+=cross(check(x),check(prev(x)));
        if(next(x)!=Map1.end()) s+=cross(check(next(x)),check(x));
        return s;
    }
    double solve2(map<double,double>::iterator x)
    {
        double s=0;
        if(x!=Map2.begin()&&next(x)!=Map2.end()) s-=cross(check(prev(x)),check(next(x)));
        if(x!=Map2.begin()) s+=cross(check(prev(x)),check(x));
        if(next(x)!=Map2.end()) s+=cross(check(x),check(next(x)));
        return s;
    }
    bool check1(point x)
    {
