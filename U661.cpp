...
struct polygon
{
    ...
    static bool polygon_cmp(point x,point y)
    {
        int z=dcmp(cross(x-Polygon1,y-Polygon1));
        return z>0||(z==0&&dcmp(disPP(x,Polygon1)-disPP(y,Polygon1))<0);
    }
    void solve()
    {
        for(int i=2;i<=n;++i)
...
struct convex
{
    int m,n;
    point a[N],b[N];
    void solve(polygon a0)
    {
        m=a0.n;
        for(int i=1;i<=m;++i) b[i]=a0.a[i];
        n=0;
        a[++n]=b[1];
        for(int i=2;i<=m;++i)
        {
