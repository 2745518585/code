...
void add(int &x,int k)
{
    ...
    if(k==T[x].k) ++T[x].t;
    else if(k<T[x].k) copy(T[x].l),add(T[x].l,k);
    else if(k>T[x].k) copy(T[x].r),add(T[x].r,k);
...
void remove(int &x,int k)
{
    ...
    if(k<T[x].k) copy(T[x].l),remove(T[x].l,k);
...
{
    if(x==0) return false;
    if(k<T[x].k) return check(T[x].l,k);
    if(k>T[x].k) return check(T[x].r,k);
    return true;
}
int num(int x,int k)
{
