...
struct Segment_tree
{
    ...
    int num(int x,int l,int r,int k)
    {
        ...
        if(l<=z) s+=num(x<<1,l,r,k);
        if(r>z) s+=num(x<<1|1,l,r,k);
        return s;
    }
    int find_f(int x,int l,int r,int k)
    {
        ...
        if(l<=z) s=max(s,find_f(x<<1,l,r,k));
        if(r>z) s=max(s,find_f(x<<1|1,l,r,k));
        return s;
    }
    int find_b(int x,int l,int r,int k)
    {
        ...
        int z=T[x].l+T[x].r>>1,s=2147483647;
        if(l<=z) s=min(s,find_b(x<<1,l,r,k));
        if(r>z) s=min(s,find_b(x<<1|1,l,r,k));
        return s;
    }
};
int main()
