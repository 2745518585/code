...
{
    if(T2[x].k==0) return;
    T2[x<<1].s=(T2[x<<1].s+(T2[x<<1].r-T2[x<<1].l+1)*T2[x].k);
    T2[x<<1].k=(T2[x<<1].k+T2[x].k);
    T2[x<<1|1].s=(T2[x<<1|1].s+(T2[x<<1|1].r-T2[x<<1|1].l+1)*T2[x].k);
    T2[x<<1|1].k=(T2[x<<1|1].k+T2[x].k);
    T2[x].k=0;
}
...
void add(int x,int l,int r,int k)
{
    ...
    {
        T2[x].s=(T2[x].s+(T2[x].r-T2[x].l+1)*k);
        T2[x].k=(T2[x].k+k);
        return;
    }
    pushdown(x);
...
int sum(int x,int l,int r)
{
    ...
    if(l<=z) s=(s+sum(x<<1,l,r));
    if(r>z) s=(s+sum(x<<1|1,l,r));
    return s;
}
