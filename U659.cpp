...
struct circle
{
    point a;
    double r;
    circle(point a0,double r0) {a=a0,r=r0;}
};
int n,n1,n2;
line a[N];
point a1[N],a2[N];
...
{
    scanf("%d",&n1);
    for(int i=1;i<=n1;++i) a1[i].input();
    a1[n1+1]=a1[1];
    scanf("%d",&n2);
    for(int i=1;i<=n2;++i) a2[i].input();
    a2[n2+1]=a2[1];
    for(int i=1;i<=n1;++i) a[++n]=line(a1[i],a1[i+1]-a1[i]);
    for(int i=1;i<=n2;++i) a[++n]=line(a2[i],a2[i+1]-a2[i]);
    Halfplane.init(n,a);
    Halfplane.solve();
