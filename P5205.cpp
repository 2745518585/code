...
struct poly
{
    ...
    friend poly operator*(poly a,ll b)
    {
        poly c;
        c.resize(a.n);
        for(int i=0;i<=c.n;++i) c.a[i]=a[i]*b%P;
        return c;
    }
    void NTT(int n,int u)
    {
        ...
        if(u==-1)
        {
            for(int i=0;i<=n;++i) a[i]=a[i]*inv(n)%P;
        }
    }
    friend poly operator*(poly a,poly b)
    {
    ...
    poly getinv(int n)
    {
        ...
        poly x=getinv(n/2).resize(n),y=((*this).copy()).resize(n);
        int p=1;
        while(p<=n*2) p<<=1;
        x.NTT(p,1);
        y.NTT(p,1);
    ...
    poly getsqrt(int n)
    {
        if(n==0) return poly(1).resize(n);
        poly x=getsqrt(n/2).resize(n);
        x=(((*this)*(x.getinv()))+x).resize(n);
        ll z=inv(2);
        for(int i=0;i<=n;++i) x.a[i]=x[i]*z%P;
        return x.resize(n);
    }
    poly getsqrt()
    {
