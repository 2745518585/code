...
namespace Print
{
    ...
    {
      public:
        print_list(std::initializer_list<std::any> list)
        {
            std::string str;
            std::stringstream stream(str);
            for(auto i:list) stream<<str;
        }
    };
    std::string print_type(const std::vector<print_list> &str_back,const std::vector<std::vector<print_list>> &str_list)
    {
        std::string str;
