...
int main()
{
    scanf("%d%lld%lld%lld%lld%lld",&n,&t1,&t2,&w0,&w1,&w2);
    s1=n*2+1;
    s2=n*2+2;
    for(int i=1;i<=n;++i)
    {
    ...
    {
        road(n+i,i+t1+1,1e18,w1);
        road(i+t1+1,n+i,0,-w1);
    }
    for(int i=1;i<=n-t2-1;++i)
    {
        road(n+i,i+t2+1,1e18,w2);
        road(i+t2+1,n+i,0,-w2);
    }
    while(SPFA())
    {
        for(int i=1;i<=s2;++i) 
